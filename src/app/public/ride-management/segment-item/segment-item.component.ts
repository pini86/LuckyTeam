...
export class SegmentItemComponent implements OnInit {
  ...
  protected readonly _segments = signal<Segments[]>([]);
  protected readonly _ride = signal<RideModel>(null);
  ...
  public ngOnInit(): void {
    ...
    // this._ride.set(this.ride());

    this._ride$.pipe(tap((t) => {
      const schedule = t.schedule.find(sch => sch.rideId === this.rideId());
      this._ride.set(t);
      this._segments.set(schedule.segments);
    })).subscribe();
  ...
  protected _submit(): void {
    ...
    // this._segments.set(modifySegment);


    const newRide: RideModel = {
      id: this._ride().id,
      path: this._ride().path,
      carriages: this._ride().carriages,
      schedule: this._ride().schedule.map((schedule, index) => {
        if (index === this.index()) {
          return {
    ...
    this._rideService.updateRide(this.routeId().toString(), this.rideId().toString(), this._segments(), newRide);
